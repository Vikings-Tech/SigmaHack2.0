...
public class echoAR : MonoBehaviour
{
   ...
   IEnumerator QueryDatabase(string serverURL,string reqModel)
    {
        ...
        if (!json.ToLower().Contains("not found"))
        {
            ...
            // Download Assets
            StartCoroutine(DownloadAssets(serverURL,reqModel));
        }
    }
    ...
    IEnumerator DownloadAssets(string serverURL,string reqModel)
    {
    ...
    public void ModelSearch(string modelReq)
    {
        // Debug logs control
#if UNITY_EDITOR
        ...
        try
        {
            StartCoroutine(QueryDatabase(serverURL,modelReq));
        }
