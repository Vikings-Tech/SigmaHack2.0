            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        if (alt.confidence < 0.5f && res.final)
                        {
                            ResultsField.text = "Speak again";
                        }
                        else if (res.final)
                        {
                            if (text.ToLower().Contains("next"))
                            {
                                ResultsField.text = "Next Step";
                                Debug.Log("Next Step");
                                _animationHandler.Next();

                            }
                            else if (text.ToLower().Contains("previous") || text.ToLower().Contains("back"))
                            {
                                ResultsField.text = "Previous Step";

                                Debug.Log("Previous Step");
                                _animationHandler.Back();
                            }
                            else if (text.ToLower().Contains("restart"))
                            {
                                ResultsField.text = "Restart";
                                Debug.Log("Restart");
                                _animationHandler.Restart();
                            }
                        }
                        else
                        {
